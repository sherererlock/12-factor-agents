...
> #### 在任何时刻，你在 Agent 中输入给 LLM 的内容都是"到目前为止发生了什么，下一步是什么"
...
本指南的全部内容是关于如何从当今的模型中获取尽可能多的价值。特别没有提及的是：

- 更改模型参数，如 temperature、top_p、frequency_penalty、presence_penalty 等。
- 训练你自己的补全或嵌入模型
- 微调现有模型

同样，我不知道将上下文传递给 LLM 的最佳方式是什么，但我知道你希望能够灵活地尝试一切。
...
虽然这对大多数用例来说效果很好，但如果你真的想从当今的 LLM 中获得最大价值，你需要以最高效利用 token 和注意力的方式将上下文输入到 LLM 中。

作为标准消息格式的替代方案，你可以构建针对你的用例优化的自定义上下文格式。例如，你可以使用自定义对象，并根据需要将它们打包/分散到一个或多个 user、system、assistant 或 tool 消息中。

这是一个将整个上下文窗口放入单个 user 消息的示例：
```yaml
...
模型可能会根据你提供的工具 schema 推断出你在问它"下一步是什么"，但将其纳入你的 prompt 模板中并无坏处。
...
XML 风格的格式只是一个示例——关键在于你可以为你的应用构建有意义的自定义格式。如果你有灵活性去尝试不同的上下文结构，以及决定存储什么与传递给 LLM 什么，你将获得更好的质量。
...
4. **灵活性**：随着你了解什么最适合你的用例来调整格式
5. **Token 效率**：针对 token 效率和 LLM 理解优化上下文格式

上下文包括：prompt、指令、RAG 文档、历史记录、工具调用、记忆


记住：上下文窗口是你与 LLM 的主要接口。掌控你组织和呈现信息的方式可以显著提升你的 Agent 的性能。
...
在 12-factor agents 发布大约两个月后，上下文工程开始成为一个非常流行的术语。
...
还有来自 [@lenadroid](https://x.com/lenadroid) 在 2025 年 7 月制作的一份相当不错的[上下文工程速查表](https://x.com/lenadroid/status/1943685060785524824)。
...
这里反复出现的主题是：我不知道什么是最好的方法，但我知道你希望能够灵活地尝试一切。


[← 掌控你的 Prompt](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-02-own-your-prompts.md) | [工具就是结构化输出 →](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-04-tools-are-structured-outputs.md)
